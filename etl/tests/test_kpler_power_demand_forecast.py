"""Kpler power-demand-forecast connector contract test — fixture-based, no live network, no DB.

Covers the (zone, model)→canonical mapping carrying `made_on`, that multiple vintages of one
delivery hour coexist (the whole point of the forecast store), the unknown zone/model drop, the
null drop, the MW sanity band, the retention rule (`_vintages_to_delete`), and the fetch
...
from gasbalance_etl.connectors.kpler_power_demand_forecast import connector as kp
from gasbalance_etl.validation.forecast_covariate import (
    forecast_covariate_demand_schema as schema,
)


def test_code_includes_zone_and_model() -> None:
    assert kp._code("FR", "EC_46") == "KP.LOADFC.FR.EC_46"
    assert kp._code("DE", "EC_AIFS_ENS") == "KP.LOADFC.DE.EC_AIFS_ENS"
...
def test_series_dict_has_two_models_per_zone() -> None:
    sd = kp.series_dict()
    codes = {e["code"] for e in sd}
    assert {"KP.LOADFC.FR.EC_46", "KP.LOADFC.DE.EC_AIFS_ENS"} <= codes
    assert len(sd) % 2 == 0  # exactly 2 models per zone
    assert {e["sub_group"] for e in sd} == {"demand"}
    assert {e["group"] for e in sd} == {"demand_forecast"}
    assert {e["model"] for e in sd} == {"EC_AIFS_ENS", "EC_46"}


def test_to_canonical_maps_zone_model_and_carries_made_on() -> None:
    df = kp.to_canonical(_raw("FR", "EC_46", 43168.0))
    assert df.loc[0, "series_id"] == "KP.LOADFC.FR.EC_46"
    assert df.loc[0, "sub_group"] == "demand"
    assert df.loc[0, "area"] == "FR"
    assert df.loc[0, "group"] == "demand_forecast"
    assert (df["source"] == "kpler_power_demand_forecast").all()
...
def test_multiple_vintages_of_same_hour_coexist() -> None:
    ...
    a = kp.to_canonical(_raw("FR", "EC_46", 43000.0, made_on="2026-06-23"))
    b = kp.to_canonical(_raw("FR", "EC_46", 44000.0, made_on="2026-06-24"))
    df = pd.concat([a, b], ignore_index=True)
    schema.validate(df, lazy=True)


def test_unknown_zone_is_dropped() -> None:
    assert kp.to_canonical(_raw("ZZ", "EC_46", 43168.0)).empty


def test_unknown_model_is_dropped() -> None:
    assert kp.to_canonical(_raw("FR", "EC_NOPE", 43168.0)).empty


def test_small_negative_is_allowed() -> None:
    # residual_demand can go negative and feeds carry small metering-noise negatives; the
    # schema's MW band must not reject them (demand_schema floors at -10_000).
    df = kp.to_canonical(_raw("FR", "EC_46", -1.0))
    schema.validate(df, lazy=True)  # no raise


def test_absurd_value_is_blocked() -> None:
    df = kp.to_canonical(_raw("FR", "EC_46", 1e9))  # e.g. a W-not-MW scale mistake
