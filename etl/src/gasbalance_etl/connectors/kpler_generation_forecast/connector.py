...
def _desired_run_dates(today: dt.date) -> list[dt.date]:
    ...
    Clamped to `_HISTORY_START` so we never request vintages that predate the data and would
    return empty forever. Fetching exactly this set means we never pull a vintage we'd prune.
    """
    ...
    return sorted(d for d in (daily | mondays) if d >= _HISTORY_START)
